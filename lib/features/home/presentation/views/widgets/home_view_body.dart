...
import 'package:fruit_hup/core/widgets/proaduct.dart';
...
class ProaductGridViewBlocBuilder extends StatelessWidget {
  const ProaductGridViewBlocBuilder({super.key});
    ...
    return BlocBuilder<ProaductsCubit, ProaductsState>(
      builder: (context, state) {
        if (state is ProaductsSuccess) {
          return ProaductGrid(proaducts: state.proaducts);
        } else if (state is ProaductsLoading) {
          return Skeletonizer.sliver(
            enabled: true,
            child: ProaductGrid(proaducts: getListOfDummyProducts()),
          );
        } else {
